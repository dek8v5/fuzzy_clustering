function [ model, anormaly] = sp1m( data_ori )

if( size(data_ori,3)>1 )
    data = rgb2gray(data_ori);
end

data = double(data_ori);
...
N = h*w; % number of data
C=5; % cluster number, input a very big number first
fzr=1.5; % fuzzifier
